...
class PushSDK {
    ...
    constructor(optiKPIPushKey, optiKPICustomerId, originURL = "https://push.optikpi.com", serviceWorkerPath = '/optikpi-service-worker.js') {
        this.optiKPIPushKey = optiKPIPushKey;
        this.optiKPICustomerId = optiKPICustomerId;
    ...
    async init() {
        try {
            const hasRequestNotificationPermission = await this.requestNotificationPermission();
    ...
    async registerPushToken(userId) {
        ...
        try{
            const registerResponse = await this.handleAPICallout(url, userId);
    ...
    async unRegisterPushToken(userId) {
        ...
        try{
            const unregisterResponse = await this.handleAPICallout(url, userId);
    ...
    async handleAPICallout(url, userId) {
        const response = await fetch(url, {
            ...
            body: JSON.stringify({
                ...
                workspaceId: this.optiKPICustomerId,   
            })
        });
        if (!response.ok) {
